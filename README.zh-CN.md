...
- 可选集成 Guardian：每轮审计使用独立绿色或红色输出块，不会成为 Agent 消息。
  暂停状态下 `c` 通过 OSC 52 复制反馈，`r` 恢复，Esc/Ctrl+C 停止。
...
Guardian 通过同一注册表增加 `/guardian status|now|history|resume`。
...
Ctrl+L 清屏，Ctrl+D 退出。
Guardian 暂停且输入框为空时，按 `c` 复制最新反馈，按 `r` 恢复守护任务。
