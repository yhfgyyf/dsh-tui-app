...
export const Config = z.object({ resume: z.string(), preset: z.string(), images: z.array(z.string()).default([]), videos: z.array(z.string()).default([]) })
...
/** Build the UI-only Audit block. Audit data never enters an agent message. */
function auditFeedback(view) {
  ...
  const lines = [
    `◆ Audit #${audit.sequence ?? view.auditSequence} · ${String(verdict).toUpperCase()} · ${audit.durationMs ?? 0} ms`,
...
const LOCAL_COMMANDS = [
  ...
  { name: '/video', desc: 'attach MP4 video · macOS clipboard: Ctrl+V', takesArg: true },
...
function imagePlaceholder(index, attachment) {
  ...
  const video = attachment?.mediaType?.startsWith('video/') === true
  const size = video || attachment?.width === undefined || attachment?.height === undefined
    ? ''
    : ` · ${attachment.width}×${attachment.height}`
  return `[${video ? 'Video' : 'Image'} #${index}]${name}${size}`
}
...
function looksLikeVideoFilename(path) {
  return /\.mp4$/iu.test(path.trim())
}
...
/** Accept either a supported raster image or an MP4 path from terminal paste. */
function pastedMediaPath(raw) {
  const normalized = normalizePastedPath(raw)
  if (normalized !== undefined && (looksLikeImageFilename(normalized) || looksLikeVideoFilename(normalized))) return normalized
  const filename = raw.trim()
  return looksLikeImageFilename(filename) || looksLikeVideoFilename(filename) ? filename : undefined
}
...
function imageMediaType(data) {
  ...
  throw new Error('unsupported media format (expected PNG, JPEG, GIF, WebP, or MP4)')
}
...
const HELP = [
  ...
  `  ${accent('/video')}      attach MP4 video to the next prompt  ${dim('(/video clip.mp4; /video clear)')}`,
  ...
  `  ${accent('/audit')} ${dim('status | now | history | accept | resume (Audit preset)')}`,
  ...
  `  ${dim('Audit review:')} ${dim('a')} execute   ${dim('e')} edit & execute   ${dim('c')} copy   ${dim('r')} resume`,
...
async function run(ctx, config, io) {
  ...
  try {
    permission = permissionLabel(permissionService?.current(sessionEvents(agent.session)))
  } catch {
  ...
  async function cyclePermission() {
    ...
    let current = 'custom'
    try {
      current = permissionService.current(sessionEvents(agent.session))
    } catch {
    ...
    try {
      ...
      status.permission = permissionLabel(permissionService.current(sessionEvents(agent.session)))
  ...
  let pendingImages = []
  let auditView
  let auditSubscription
  let lastAuditBlockId
  let auditEdit

  function renderAuditView(view, { replay = false } = {}) {
    auditView = view
    const block = auditFeedback(view)
    if (block === undefined || (!replay && block.id === lastAuditBlockId)) return
    lastAuditBlockId = block.id
    const style = block.tone === 'success' ? green : block.tone === 'error' ? red : cyan
    screen.paint(block.text.split('\n').map((line) => style(line)).join('\n') + '\n')
  }

  async function bindAudit(sessionId, { replay = false } = {}) {
    auditSubscription?.()
    auditSubscription = undefined
    auditView = undefined
    lastAuditBlockId = undefined
    auditEdit = undefined
    if (audits === undefined) return
    auditSubscription = audits.subscribe(String(sessionId), (view) => renderAuditView(view))
    try {
      const view = await audits.snapshot(String(sessionId))
      if (view?.active === true) renderAuditView(view, { replay })
    } catch (error) {
      screen.paint(red(`✖ cannot read Audit state: ${error instanceof Error ? error.message : String(error)}`) + '\n')
    }
  }

  async function copyAuditFeedback() {
    const block = auditFeedback(auditView)
    if (block === undefined) return
    screen.copyToClipboard(block.text)
    screen.paint(green('◆ Audit feedback copied to terminal clipboard') + '\n')
  }

  async function resumeAudit() {
    if (audits === undefined) return
    try {
      const view = await audits.resume(String(agent.session.id))
      auditView = view
      screen.paint(green('◆ Audit resumed') + '\n')
    } catch (error) {
      screen.paint(red(`✖ cannot resume Audit: ${error instanceof Error ? error.message : String(error)}`) + '\n')
    }
  }

  async function acceptAudit(editedText, auditId) {
    const retryable = auditView?.remediation !== undefined && ['failed', 'execution-failed', 'verification-failed'].includes(auditView.remediation.phase)
    if (audits === undefined || (auditView?.pendingApproval?.status !== 'pending' && !retryable)) return
    try {
      const selectedAuditId = auditId ?? auditView.pendingApproval.auditId
      const view = await audits.accept(String(agent.session.id), selectedAuditId, editedText)
      auditView = view
      const delivery = view.remediation?.delivery === 'next-step' ? 'after current tool call' : 'now'
      screen.paint(green(`◆ Audit remediation accepted · ${delivery} · ${view.remediation?.phase ?? 'queued'}`) + '\n')
    } catch (error) {
      screen.paint(red(`✖ cannot accept Audit remediation: ${error instanceof Error ? error.message : String(error)}`) + '\n')
    }
  }

  function editAudit() {
    const retryable = auditView?.remediation !== undefined && ['failed', 'execution-failed', 'verification-failed'].includes(auditView.remediation.phase)
    const approval = auditView?.pendingApproval
    if ((approval?.status !== 'pending' && !retryable) || approval?.auditId === undefined) return
    const editableText = auditView?.remediation?.instruction ?? approval.editableText
    if (editableText === undefined || editableText === '') {
      screen.paint(red('✖ Audit did not provide editable remediation text') + '\n')
      return
    }
    auditEdit = { auditId: approval.auditId }
    screen.setBuffer(editableText)
    screen.paint(cyan('◆ Edit Audit remediation, then press Enter to execute; Esc cancels') + '\n')
  }

  await bindAudit(agent.session.id, { replay: resumeId !== undefined })
  ...
  /** Convert an explicit terminal paste to an image or direct MP4 attachment when possible. */
  async function attachPastedImage(pasted) {
    const path = pastedMediaPath(pasted)
  ...
  async function switchToSession(target, marker, presetId = status.preset) {
    try {
      ...
      await bindAudit(agent.session.id, { replay: target !== undefined })
  ...
  async function handleLine(text) {
    ...
    if (text === '/video' || text.startsWith('/video ')) {
      ...
      if (raw === 'clear') {
        pendingImages = []
        screen.setComposerImages([])
        screen.paint(dim('  media attachments cleared') + '\n')
        return
      }
      if (raw === '') {
        const videos = pendingImages.filter((block) => block.attachment.mediaType?.startsWith('video/') === true)
        const lines = videos.length === 0
          ? [dim('  no MP4 video attached to the next prompt · macOS clipboard: press Ctrl+V (not Command+V)')]
          : videos.map((block, index) => dim(`  ${imagePlaceholder(index + 1, block.attachment)} attached`))
        screen.paint(lines.join('\n') + '\n')
        return
      }
      const paths = imagePaths(raw)
      if (paths.some((path) => !looksLikeVideoFilename(path))) {
        screen.paint(red('✖ /video accepts .mp4 files only') + '\n')
        return
      }
      try {
        await attachImagePaths(paths)
      } catch (error) {
        screen.paint(red(`✖ cannot attach video: ${error instanceof Error ? error.message : String(error)}`) + '\n')
      }
      return
    }
  ...
  await attachImagePaths(config.videos ?? [])
  ...
  if (screen.enabled) {
    ...
    process.stdin.on('keypress', (chunk, key) => {
      ...
      if (key.ctrl && name === 'c') {
        ...
        else if (auditEdit !== undefined) {
          auditEdit = undefined
          screen.clearBuffer()
          screen.paint(dim('◆ Audit edit canceled') + '\n')
        }
        else if (!screen.bufferEmpty()) screen.clearBuffer()
        else quit(0)
        return
      }
      if (!key.ctrl && !key.meta && screen.bufferEmpty() && auditView?.paused === true && name === 'c') {
        enqueueInputTask(copyAuditFeedback)
        return
      }
      const auditRetryable = auditView?.remediation !== undefined && ['failed', 'execution-failed', 'verification-failed'].includes(auditView.remediation.phase)
      if (!key.ctrl && !key.meta && screen.bufferEmpty() && (auditView?.pendingApproval?.status === 'pending' || auditRetryable) && name === 'a') {
        enqueueInputTask(acceptAudit)
        return
      }
      if (!key.ctrl && !key.meta && screen.bufferEmpty() && (auditView?.pendingApproval?.status === 'pending' || auditRetryable) && name === 'e') {
        editAudit()
        return
      }
      if (!key.ctrl && !key.meta && screen.bufferEmpty() && auditView?.paused === true && (auditView?.remediation === undefined || auditView.remediation.phase === 'completed') && auditView?.pendingApproval?.verdict !== 'critical' && name === 'r') {
        enqueueInputTask(resumeAudit)
        return
      }
      if (key.ctrl && name === 'd') {
      ...
      if (name === 'return' || name === 'enter') {
        ...
        if (auditEdit !== undefined) {
          const edit = auditEdit
          auditEdit = undefined
          const editedText = screen.submit()
          enqueueInputTask(() => acceptAudit(editedText, edit.auditId))
          return
        }
      ...
      if (name === 'escape') {
        ...
        else if (auditView?.paused === true) quit(0)
        return
      }
...
internals.looksLikeVideoFilename = looksLikeVideoFilename
internals.pastedImagePath = pastedImagePath
internals.pastedMediaPath = pastedMediaPath
...
internals.auditFeedback = auditFeedback
