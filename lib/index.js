...
export const inject = ['agentDefaultModel', 'agentPresets', 'agents', 'attachments', 'llm', 'sessions', 'sessionPersistence', 'commands', 'permissionPresets', 'skills']
...
function guardianFeedback(view) {
  ...
  if (approval?.verdict === 'warning') lines.push('  REVIEW · a accept repair · current agent continues without approval')
  if (approval?.verdict === 'critical') lines.push('  PAUSED · a accept repair · c copy feedback · Esc/Ctrl+C stop')
  else if (retryable) lines.push('  PAUSED · a retry repair · c copy feedback · Esc/Ctrl+C stop')
...
const HELP = [
  ...
  `  ${dim('Guardian review:')} ${dim('a')} accept repair   ${dim('c')} copy feedback   ${dim('r')} resume non-review pause`,
...
async function run(ctx, config, io) {
  ...
  if (agents === undefined || defaultModel === undefined || presets === undefined || attachments === undefined || llm === undefined || sessions === undefined || persistence === undefined || rootSkills === undefined) return
  ...
  async function acceptGuardian() {
    ...
    try {
      const view = await guardians.accept(String(agent.session.id), guardianView.pendingApproval.auditId)
      guardianView = view
      screen.paint(green(`◆ Guardian remediation accepted · ${view.remediation?.phase ?? 'queued'}`) + '\n')
    } catch (error) {
  ...
  async function deleteSessionById(id) {
    try {
      ...
      // then uses the public DSH backend seam instead of its private findLog().
