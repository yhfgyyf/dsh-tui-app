 ...
 * completion covering both the local commands and the app registry's slash
 * commands (the web surface's /compact /goal /feedback /export …).
...
export const inject = ['agentDefaultModel', 'agentPresets', 'agents', 'attachments', 'llm', 'sessions', 'sessionPersistence', 'commands', 'permissionPresets']
...
const LOCAL_COMMANDS = [
  ...
  { name: '/image', desc: 'attach images to the next prompt', takesArg: true },
...
function makeScreen(io, status, initialMenuCommands = LOCAL_COMMANDS) {
  ...
  function layout() {
    ...
    rows.push({ text: row(` ${accentBold('❯')} ${win.text}`), input: true, caret: 4 + win.caret })
    rows.push({ text: borderRow('╰', '╯'), input: false })
...
/** Identify one of rc.8's supported raster formats from its encoded signature. */
...
/** Match rc.8's host preflight: explicit omission is negative; unknown is allowed. */
...
async function run(ctx, config, io) {
  ...
  if (agents === undefined || defaultModel === undefined || presets === undefined || attachments === undefined || llm === undefined || sessions === undefined || persistence === undefined) return
  ...
  /** List the models the current provider advertises (advisory). */
  async function listModels() {
    const llm = ctx.get('llm')
    try {
      return await llm?.listModels(holder.current.provider) ?? []
    } catch {
      return []
    }
  }
  ...
  /** Apply a model switch to the default selection and the live agent. */
  async function applyModel(model) {
    await defaultModel.saveSelection({
      provider: holder.current.provider,
      model,
      ...holder.current.reasoningEffort === void 0 ? {} : { reasoningEffort: String(holder.current.reasoningEffort) }
    })
  ...
  try {
    commands?.register({
      name: 'model',
      description: 'Switch the model (↑/↓ pick from the list, or /model <id>)',
      input: { hint: '<id>' },
      handler: async (invocation) => {
        ...
        if (requested !== '') {
          ...
          if (models.length > 0 && !models.some((model) => model.id === requested)) {
            return { kind: 'error', text: `unknown model "${requested}" (available: ${models.map((model) => model.id).join(', ')})` }
          }
          return await applyModel(requested)
        }
        const models = await listModels()
        if (models.length === 0) return { kind: 'error', text: 'no models advertised by the provider' }
        if (!screen.enabled) {
          // piped: print the list instead of opening an interactive picker
          const current = defaultModel.currentSelection().model
          const lines = models.map((model) => `  ${model.id === current ? accent('▸') : ' '} ${cyan(model.id)}${model.name !== undefined && model.name !== model.id ? dim(` · ${model.name}`) : ''}`)
          return { kind: 'success', text: [dim(`models (${holder.current.provider})`), ...lines, dim('  /model <id> to switch')].join('\n') }
        }
        const current = defaultModel.currentSelection().model
        const picked = await pickerPrompt(models.map((model) => ({
          name: model.id,
          desc: `${model.name !== undefined && model.name !== model.id ? `${model.name} · ` : ''}${model.id === current ? 'current' : ''}`,
          value: model.id
        })), `models (${holder.current.provider})`)
  ...
  const registryCommands = (commands?.list(agent) ?? [])
  ...
  const menuCommands = [...LOCAL_COMMANDS, ...registryCommands].sort((left, right) => (left.name < right.name ? -1 : 1))

  const screen = makeScreen(io, status, menuCommands)
  let turnActive = false
  let pendingImages = []

  /** Attach one validated batch to the next prompt, mirroring Codex CLI -i. */
  async function attachImagePaths(paths) {
    if (paths.length === 0) return
    const limits = attachments.imageLimits
    if (pendingImages.length + paths.length > limits.maxImagesPerMessage) {
  ...
  function refreshMenuCommands() {
    const registry = (commands?.list(agent) ?? [])
      .map((command) => ({ name: `/${command.name}`, desc: command.description, takesArg: command.input !== undefined }))
      .filter((item) => !LOCAL_COMMANDS.some((local) => local.name === item.name))
    screen.setMenuCommands([...LOCAL_COMMANDS, ...registry].sort((left, right) => (left.name < right.name ? -1 : 1)))
  }
  ...
  async function deleteSessionById(id) {
    try {
      ...
      // inspect waits for a disposed lifecycle's persistence retirement. locate
      // then uses the public rc.8 backend seam instead of its private findLog().
      const inspected = await persistence.inspect(id)
      const location = persistence.locate(inspected.meta)
      if (location?.kind !== 'jsonl') throw new Error('the configured session backend does not expose a deletable per-session artifact')
  ...
  async function handleLine(text) {
    ...
    if (text === '/image' || text.startsWith('/image ')) {
      ...
      if (raw === '') {
        const lines = pendingImages.length === 0
          ? [dim('  no images attached to the next prompt')]
    ...
    if (commands !== undefined && commands.find(agent, commandName) !== undefined) {
      try {
        ...
        if (result?.kind === 'success') pendingImages = []
      } catch (error) {
