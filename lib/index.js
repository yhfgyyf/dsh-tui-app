...
export const inject = ['agentDefaultModel', 'agentPresets', 'agents', 'sessions', 'sessionPersistence', 'commands', 'permissionPresets']

export const Config = z.object({ resume: z.string(), preset: z.string() })
...
async function run(ctx, config, io) {
  ...
  if (agents === undefined || defaultModel === undefined || presets === undefined || sessions === undefined || persistence === undefined) return
  ...
  async function startAgent(options) {
    ...
    return { agent: created.agent, presetId }
  }
  ...
  /** Remove a session from the shared persisted store (web app sees it vanish too). */
  async function deleteSessionById(id) {
    try {
      ...
      const logPath = await persistence.findLog(id)
      if (logPath === undefined) {
        if (wasCurrent) {
          screen.paint(`${dim('deleted')} ${cyan(shortId(id))}\n`)
          return
        }
        screen.paint(red(`✖ session ${shortId(id)} has no stored log`) + '\n')
        return
      }
  ...
  async function handleLine(text) {
    ...
    if (commands !== undefined && commands.find(agent, commandName) !== undefined) {
      try {
        const outcome = await commands.execute(agent, text, new AbortController().signal)
    ...
    // a chat turn: echo the prompt, then stream the reply above the bar
    screen.paint(`${accentBold('❯')} ${text}\n`)
    status.step = 0
    status.in = 0
    status.out = 0
    status.cache = 0
