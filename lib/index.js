...
function guardianFeedback(view) {
  ...
  if (view.paused) lines.push('  PAUSED · c copy feedback · r resume · Esc/Ctrl+C stop')
  return { id: String(audit.id ?? audit.sequence), tone, text: lines.join('\n') }
}
...
const HELP = [
  ...
  `  ${accent('/guardian')} ${dim('status | now | history | resume (Guardian preset)')}`,
  ...
  `  ${dim('Guardian paused:')} ${dim('c')} copy feedback   ${dim('r')} resume   ${dim('esc/ctrl+c')} stop`,
...
async function run(ctx, config, io) {
  ...
  if (screen.enabled) {
    ...
    process.stdin.on('keypress', (chunk, key) => {
      ...
      if (!key.ctrl && !key.meta && screen.bufferEmpty() && guardianView?.paused === true && name === 'r') {
