...
function tuiCommand() {
  return new Command()
    ...
    .option('-p, --preset <id>', 'agent preset for a new session (standard, code, minimal, cordis; auto/guardian when installed)')
    .option('-i, --image <path>', 'attach an image to the first prompt (repeat or use comma-separated paths)', collectImagePaths, [])
    .option('-v, --video <path>', 'attach an MP4 video to the first prompt (repeat or use comma-separated paths)', collectImagePaths, [])
    .addHelpText('after', `
Examples:
  ...
  dsh --profile tui --preset guardian start with independent Guardian review
  dsh --profile tui --preset minimal start a new session in minimal mode
  dsh --profile tui -i diagram.png   attach an image to the first prompt
  dsh --profile tui -i a.png -i b.jpg attach multiple images
  dsh --profile tui -v clip.mp4      attach an MP4 video directly
  dsh --profile tui --resume <id>    resume an existing session
`)
}

/** Commander collector matching Codex CLI's repeatable/comma-separated -i flag. */
export function collectImagePaths(value, previous = []) {
  return [
    ...previous,
    ...value.split(',').map((path) => path.trim()).filter((path) => path !== '')
  ]
}
